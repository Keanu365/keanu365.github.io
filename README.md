# 👋 Hey there! I'm Keanu365!
You can go visit my main account [here](github.com/Keanu365)!

![Profile Picture](Keanu.jpg)

**Other names I go by:**
* Thet Paing Min (my real name)
* TheCoconutMan (a name you'll commonly find on my other socials)
* Keanu Min (my "nickname")

### 🏫 I'm currently studying at _NUS High School of Mathematics and Science_. (That's in _Singapore_!)

### Languages I've learnt/currently learning:
* Python (my very first text-based language! 🐍)
* JavaScript (mostly for web development!)
* C++ (I learnt a bit to do Arduino some time ago 👀)
* C# (Learnt a bit for Unity!)
* Lua (I dabbled in Roblox Studio for a while too!)
* Java (though I might be a bit rusty now lol)
* Kotlin (for Android development!)
🐧 I also learnt a bit of _Linux_ not too long ago while studying cybersecurity.

### Some of my proudest works on GitHub:
(You can also go look at a personal webpage I made when I was much younger [here](https://keanu365.github.io/Keanu/).)
* [ClassMate](https://github.com/Keanu365/ClassMate) - helps you generate UML Diagrams from your Java code! Made in Java, for Java!
  * This was made with some help from my good friends [soyuz_dev](https://github.com/soyuz-dev) and [looeydooey866](https://github.com/looeydooey866)! Go check them out too!
* Coming soon - StudBud! Stay tuned!
* Go check out some of my other works/repositories [here](https://github.com/Keanu365?tab=repositories)!

### 🌐 Some of my other socials:
* [▶️ YouTube](https://www.youtube.com/@TheCoconutMan10)
* [📸 Instagram](https://www.instagram.com/thecocoman2010/) (I barely use this though)
* [📧 Email](mgkeanu@gmail.com) (You can contact me through email!)
* [😺 Scratch](https://scratch.mit.edu/users/SmartKeanu/) (If you feel like laughing at younger me)
* [🫃 Reddit](https://www.reddit.com/user/AnonymousSeally/) (I... honestly don't know why you'd even need my Reddit but here you go)
